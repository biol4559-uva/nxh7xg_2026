...
# this doesn't work because R can't read the whole equation -> it needs it in parts
# p1 (aka AA) = p^2 BUT only in terms of one variable so (1-q)^2
hwe_fun <- function(p, q) {
  p^2 + 2*p*q + q^2 = 1
  }

out_hwe <- hwe_fun(p=c(0:1), q=c(0:1))

hwe_fun <- function(q, p) {
  AA <- (1-q)^2
  Aa <- 2*(1-q)*q
  aa <- q^2
  return(data.table(q=q, AA=AA, Aa=Aa, aa=aa))
}

out_hwe <- hwe_fun(q=c(0:100)/100)

outhwe_long <- melt(out_hwe, id.vars="q", measure.vars=c("AA", "Aa", "aa"))

p1 <- ggplot(data=outhwe_long) +
  geom_line(aes(x=q, y=value, group=variable, color=variable))
...
## generate data
outhwe <- hwe(p=c(0:100)/100)

out_long <- melt(outhwe, id.vars=c("p"), measure.vars=c("fAA", "fAa", "faa"))

p2 <- ggplot(data=out_long) +
